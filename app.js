...
function createButtonConcluir(tarefa) {
  ...
  if (tarefa.estahConcluida) {
    btnConcluir.innerHTML = 'Desfazer';
  } else {
...
function buttonCadastrarTarefaClick() {
  ...
  var entrada_titulo = document.querySelector("#entrada-titulo");
  var entrada_data = document.querySelector("#entrada-data");
  if(entrada_titulo.value != "" && entrada_data.value != ""){
    data = entrada_data.value.split("-");
    if(editando){
      tarefaEditada.titulo = entrada_titulo.value;
      tarefaEditada.data = new Date(data[0],data[1],data[2]);
    }else{
      salvarTarefa(entrada_titulo.value,new Date(data[0],parseInt(data[1])-1,data[2]),false);
    }
    entrada_titulo.value = null;
    entrada_data.value = null;
    apresentarTarefas();
  }else{
    alert("campos não pode está vazio!");
  }
}
...
function buttonEditarTarefaClick() {
  ...
  entrada_titulo.value = tarefa.titulo;
  data = [tarefa.dataDeTermino.getDate(),tarefa.dataDeTermino.getMonth()+1,tarefa.dataDeTermino.getFullYear()]
  entrada_data.value= data[2]+"-0"+data[1]+"-"+data[0];
  editando = true;
...
function createApp() {
  ...
  hSubtitle.innerHTML = 'Demonstração de recursos de manipulação do DOM com JavaScript - Douglas Aquino Moreno';
  app.appendChild(hSubtitle);

  var form_cadastro = document.createElement('form');
  form_cadastro.id = "form-cadastro";

  var entrada_titulo = document.createElement('input');
  entrada_titulo.type = "text";
  entrada_titulo.placeholder = "titulo da tarefa";
  entrada_titulo.id = "entrada-titulo";
  form_cadastro.appendChild(entrada_titulo);
  
  var entrada_data = document.createElement('input');
  entrada_data.type = "date";
  entrada_data.id = "entrada-data";
  form_cadastro.appendChild(entrada_data);
  ...
  form_cadastro.appendChild(buttonCadastrarTarefa);
  
  app.appendChild(form_cadastro);
  ...
  thConcluir.innerHTML = 'Ações Tarefas';
